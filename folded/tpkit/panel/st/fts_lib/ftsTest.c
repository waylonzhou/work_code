...
int production_test_ms_cx(char *path_limits, int stop_on_fail, TestToDo *todo,struct ts_rawdata_info *info,TestResult *result) {
	...
	if (todo->MutualCx2) {
		...
		ret = checkLimitsMap((short *)msCompData.node_data, msCompData.header.force_node, msCompData.header.sense_node, thresholds_min, thresholds_max); //check the limits
		if (ret != OK) {
...
int production_test_ms_key_cx(char *path_limits, int stop_on_fail, TestToDo *todo) {
	...
	if (todo->MutualKeyCx2 == 1) {
		...
		ret = checkLimitsMap((short*)msCompData.node_data, 1, num_keys, thresholds_min, thresholds_max); //check the limits
		if (ret != OK) {
	...
	if (todo->MutualKeyCxTotal == 1) {
		ret = computeTotal((short*)msCompData.node_data, msCompData.cx1, 1, num_keys, CX1_WEIGHT, CX2_WEIGHT, &total_cx);
		if (ret < 0) {
