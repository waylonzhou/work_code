...
//cat mode_active		    to show the bitmask of which indicate the modes/features which are running on the IC in a specific instant of time (example output in the terminal = "AA10000000BB" only senseOn performed)
...
static ssize_t fts_strength_frame_show(struct device *dev, struct device_attribute *attr, char *buf){
	...
	int res= ERROR_OP_NOT_ALLOW, count=0, j, size =6*2;
	u16 type = 0;
...
static ssize_t stm_fts_cmd_show(struct device *dev, struct device_attribute *attr,
    ...
    todoDefault.MutualRawLP = 1;
    todoDefault.MutualRawGapLP = 0;
    todoDefault.MutualRawAdjLP = 0;
	...
	res = fb_unregister_client(&info->notifier);
        ...
        switch (typeOfComand[0]) {
            ...
            case 0x00:
#ifdef GOLDEN_VALUE
		logError(0, "%s Golden value approach! \n", tag, res);
		if(ftsInfo.u8_msScrConfigTuneVer!=ftsInfo.u8_msScrCxmemTuneVer){
			...
			res = production_test_main(LIMITS_FILE, 1, init_type, &todoDefault, INIT_MP);   
#else
...
static int cx_crc_check(void) {
    ...
    if ( res >= OK) {
		if(readData[2] == CRC_CONFIG_SIGNATURE || readData[2] == CRC_CONFIG || readData[2] == CRC_CX_MEMORY){
...
static void fts_fw_update_auto(struct work_struct *work) {
#ifndef FTM3_CHIP
    u8 cmd[4] = { FTS_CMD_HW_REG_W, 0x00, 0x00, SYSTEM_RESET_VALUE };
    int event_to_search[3] = {(int)EVENTID_ERROR_EVENT, (int)EVENT_TYPE_CHECKSUM_ERROR,-1};
    u8 readData[FIFO_EVENT_SIZE];
    int flag_init = NO_INIT;
#ifdef GOLDEN_VALUE
    int keep_cx =0;
#else
    int keep_cx =1;
	...
	logError(1,"%s %s: No CX CRC Found! \n",tag, __func__);
		...
		if(readData[2] == CRC_CX_MEMORY){
			logError(1,"%s %s: Warning CRC Error for CX found,use golden value methed so the touch may cannot work normal\n",tag, __func__, readData[2]);
			flag_init=NO_INIT_CX_ERROR;
		}
		if(readData[2] == CRC_PANEL_CONFIG){
			event_to_search[2]=CRC_CX_MEMORY;
			 ret = pollForEvent(event_to_search, 3, readData, GENERAL_TIMEOUT);
			if ( ret < OK) {
				logError(1,"%s %s: CRC PANEL  found!,need do panel init\n",tag, __func__);
				flag_init=SPECIAL_PANEL_INIT;
			}else{
				logError(1,"%s %s: Warning CRC Error for CX found,use golden value methed so the touch may cannot work normal\n",tag, __func__, readData[2]);
				flag_init=NO_INIT_CX_ERROR;
			}
...
#endif
#ifdef GOLDEN_VALUE
	if(flag_init == NO_INIT_CX_ERROR){
		logError(1,"%s %s: CRC  CX error try recovery it form fw file.but not sure if can success\n",tag, __func__);
 		retval1 = flashProcedure(PATH_FILE_FW, CRC_CX_MEMORY, 0);
		
	}
	if( (flag_init == NO_INIT) &&  (ftsInfo.u8_msScrConfigTuneVer!=ftsInfo.u8_msScrPnlCfgTuneVer))
	 	flag_init =SPECIAL_PANEL_INIT;
	
	if(flag_init ==SPECIAL_PANEL_INIT){
		error =production_test_initialization(flag_init);
		 if (error < OK) {
           	 	logError(1, "%s % special panel init failed ERROR %08X\n", tag, __func__, error);
        	}
	}
#else
...
static int fts_chip_initialization(struct fts_ts_info *info) {
    ...
    for (retry = 0; retry <= INIT_FLAG_CNT; retry++) {
        ret2 = production_test_main(LIMITS_FILE, 1, init_type, &todoDefault, INIT_FIELD);
        if (ret2 == OK)
            break;
        initretrycnt++;
