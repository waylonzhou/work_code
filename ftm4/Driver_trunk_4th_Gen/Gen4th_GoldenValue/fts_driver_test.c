...
static ssize_t stm_driver_test_show(struct device *dev, struct device_attribute *attr, char *buf) {
    ...
    if (numberParam >= 1) {
	    ...
	    break;
            ...
            case CMD_INITTEST:
#ifdef GOLDEN_VALUE			
                if ((numberParam == 3) && (functionToTest[2] ==0x01 || functionToTest[2] ==0x02 )){ 
#else
                if ((numberParam == 3)&& (functionToTest[2] ==0x00) ||(functionToTest[2] ==0x01)){ 
#endif				
                    if (functionToTest[1] == 0x01){
                        res = production_test_initialization(functionToTest[2]);
                    }else{
