...
typedef struct {								   
	...
	u8 u8_msScrPnlCfgTuneVer;      		   ///< 81 - MS screen tuning version in panel config

	u32 u32_mpPassFlag;		       ///< 82 - Mass production pass flag
	u32 u32_featEn;                        ///< 86 - Supported features
	u32 u32_echoEn;				///< 8A - enable of particular features: first bit is Echo Enables
	u8 u8_sideTchConfigTuneVer;		///< 8E - Side Touch tuning version in config
	u8 u8_sideTchCxmemTuneVer;		///< 8F - Side Touch tuning version in cxmem
	u8 u8_sideTchForceLen;			///< 90 - Number of force channel on side touch
	u8 u8_sideTchSenseLen;			///< 91 - Number of sense channel on side touch
	u8 u64_sideTchForceEn[8];		///< 92 - Side touch force channel enable
	u8 u64_sideTchSenseEn[8];		///< 9A - Side touch sense channel enable
	u8 u8_errSign;				///< A2 - Signature for error field
	u16 u16_errOffset;		   	///< A3 - Error Offset
} chipInfo;
