...
int production_test_initialization(int type) {
    int res;
	int cmd_length=0;
	int poll_event_length=0;
	u8 cmd[2]={FTS_CMD_FULL_INITIALIZATION, FTS_CMD_PANEL_INIT};
    u8 readData[FIFO_EVENT_SIZE];
	int eventToSearch[3] = {EVENTID_STATUS_UPDATE, EVENT_TYPE_FULL_INITIALIZATION,0x02};

	logError(0, "%s INITIALIZATION command sent, the init type is:%d \n", tag,type);
	if(type == FULL_PANEL_INIT){
		cmd_length=1;
		poll_event_length=2;
	}else if(type == SPECIAL_PANEL_INIT){
		cmd_length=2;
		poll_event_length=3;
	}else{
		logError(0, "%s INITIALIZATION type is not support\n", tag);
		return ERROR_PROD_TEST_INITIALIZATION;
	}
    res = fts_system_reset();
    if (res < 0) {
	...
	if (fts_writeFwCmd(cmd, cmd_length) < 0) {
	...
	res = pollForEvent(eventToSearch, poll_event_length, readData, TIMEOUT_INITIALIZATION_TEST_RESULT);
    if (res < 0) {
        logError(1, "%s production_test_initialization: INITIALIZATION Production test failed... ERROR %02X\n", tag, ERROR_PROD_TEST_INITIALIZATION);
        return (res | ERROR_PROD_TEST_INITIALIZATION);
    }

	if ((poll_event_length == 2 && readData[2] != 0x00)  || (poll_event_length == 3 && readData[3] != 0x00)) {
...
int save_cx_mpflag(int type) {
    int res;
    u8 cmd;
    u8 readData[FIFO_EVENT_SIZE];
    int eventToSearch[2] = {EVENTID_STATUS_UPDATE, EVENT_TYPE_COMP_DATA_SAVED};

    logError(0, "%s SAVE type command sent is %d... \n", tag,type);
	if((type !=SAVE_CX_TUNE) && (type != SAVE_MP_FLAG))
	{
		 logError(0, "%s SAVE type not supported: %d... \n", tag,type);
		 return ERROR_SAVE_CX_TUNING;
	}
	if(type == SAVE_CX_TUNE)
    cmd = FTS_CMD_SAVE_CX_TUNING;
	else(type == SAVE_MP_FLAG)
	...
	if((type == SAVE_CX_TUNE) && (readData[2] != 0x00 || readData[3] != 0x00)){
        logError(0, "%s SAVE CX finished!.................FAILED  ERROR %02X\n", tag, ERROR_SAVE_CX_TUNING);
        res = ERROR_SAVE_CX_TUNING;
	}else if((type == SAVE_MP_FLAG) && (readData[2] != 0x01 || readData[3] != 0x00)){
        logError(0, "%s SAVE MP FLAG finished!.................FAILED  ERROR %02X\n", tag, ERROR_SAVE_CX_TUNING);
        res = ERROR_SAVE_CX_TUNING;
    } else {
        logError(0, "%s SAVE CX or MP flag finished!.................OK\n", tag);
        res = OK;
    }
        ...
        if (res < 0) {
            ...
            if (res < 0) {
                ...
                if (saveToFlash) {
                    ...
                    res = save_cx_mpflag(SAVE_CX_TUNE);
                    if (res < 0) {
...
int production_test_main(char * pathThresholds, int stop_on_fail, int saveInit, TestToDo *todo, u32 signature) {
    ...
    if (saveInit != 0){
        res = production_test_initialization(saveInit);
	if (res < 0) {
...
int production_test_ms_raw_lp(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	{
		ret = getMSFrame2(MS_TOUCH_LOW_POWER, &msRawFrame);
		if (ret < 0)
		{
		...
		{
			ret = parseProductionTestLimits(path_limits, MS_RAW_LP_MIN_MAX, &thresholds, &trows, &tcolumns);
		...
		{
			ret = parseProductionTestLimits(path_limits, MS_RAW_LP_GAP, &thresholds, &trows, &tcolumns);
		...
		if (todo->MutualRawAdjLP == 1)
		{
			...
			ret = parseProductionTestLimits(path_limits, MS_RAW_LP_ADJH, &thresholds, &trows, &tcolumns);
			...
			ret = parseProductionTestLimits(path_limits, MS_RAW_LP_ADJV, &thresholds, &trows, &tcolumns);
...
int save_mp_flag(u32 signature){
	...
	for(i = 0; i<SAVE_FLAG_RETRY && res<OK; i++){
		...
		if(res>=OK)
			res = save_cx_mpflag(SAVE_MP_FLAG);
