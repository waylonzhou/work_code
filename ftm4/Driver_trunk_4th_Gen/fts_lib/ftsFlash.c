...
#include "ftsTime.h"
#include "ftsTool.h"
				//needed for including the define FW_H_FILE  
...
//#ifdef GOLDEN_VALUE
	if(i==GOLDEN_PANEL_INIT_PAGE){
		logError(0, "%s Skipping panel init erase page %d! \n", tag,i);
		continue;
	}
//#endif
...
/*int flash_erase_page_by_page_info(int page) {
    ...
    u8 status,i=0;
    u8 cmd[4] = {FLASH_CMD_WRITE_REGISTER, FLASH_ERASE_CODE0, 0x00, 0x40}; //write the command to erase the flash
...
int flash_burn(Firmware fw, int force_burn,int keep_cx) {
	...
	for(res = EXTERNAL_RELEASE_INFO_SIZE-1; res >=0; res--){
		...
		if (force_burn == CRC_CX_MEMORY && fw.sec2_size == 0) {
...
#ifdef GOLDEN_VALUE
    if (fw.sec2_size != 0 &&(keep_cx == 0 || force_burn == CRC_CX_MEMORY)) {
	 	logError(0, "%s 7.5) LOAD CX:\n", tag);
