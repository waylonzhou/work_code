...
static LimitFile limit_file;	/* /< variable which contains the limit file
				 * during test */
TestItemStatus test_item_check;
/**
...
int initTestToDo(void)
{
	...
	test_item_check.check_switch=SWITCH_IDLE;
	test_item_check.node_status = (u16*)kmalloc(10*sizeof(u16),GFP_KERNEL);
	if(test_item_check.node_status == NULL){
		logError(1, "%s test_item_check.node_status: ERROR %08X\n", tag,ERROR_ALLOC);
		return ERROR_ALLOC;
	}
	test_item_check.data= (int*)kmalloc(10*sizeof(int),GFP_KERNEL);
	if(test_item_check.data == NULL){
		logError(1, "%s test_item_check.data: ERROR %08X\n", tag,ERROR_ALLOC);
		return ERROR_ALLOC;
	}
...
int production_test_ito(char *path_limits, TestToDo *todo)
{
	...
	if (todo->MutualRawAdjITO == 1) {
		...
		res = checkLimitsMapAdjTotal(adj, msRawFrame.header.force_node,
					     ...
					     thresholds);
		if (res != OK) {
			if(test_item_check.check_switch == SWITCH_ON){
				test_item_check.node_status[ITO_ITEM_CHECK] = ((res& 0x3FF) | 0x400);
			}
			logError(1,
		...
		if (res != OK) {
			if(test_item_check.check_switch == SWITCH_ON){
				test_item_check.node_status[ITO_ITEM_CHECK] = ((res& 0x3FF) | 0x800);
			}
			logError(1,
			 ...
			 TestToDo *todo, u8 mpflag)
{
	...
	if (res < 0) {
		if(test_item_check.check_switch == SWITCH_ON)
			test_item_check.node_status[ITO_ITEM_CHECK] |= ITEM_FAILED;
		logError(0, "%s Error during ITO TEST! ERROR %08X\n", tag, res);
		goto END;/* in case of ITO TEST failure is no sense keep going
			 * */
	} else{
		if(test_item_check.check_switch == SWITCH_ON)
			test_item_check.node_status[ITO_ITEM_CHECK] = ITEM_OK;
	...
	if (saveInit != NO_INIT) {
		...
		if (res < 0) {
			if(test_item_check.check_switch == SWITCH_ON)
				test_item_check.node_status[INITIALIZATION_ITEM_CHECK] = ITEM_FAILED;
			logError(0,
			...
			if(test_item_check.check_switch == SWITCH_ON)
				test_item_check.node_status[INITIALIZATION_ITEM_CHECK] = ITEM_OK;
...
int production_test_ms_raw(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	if (todo->MutualRaw == 1 || todo->MutualRawGap == 1 ||
		...
		if (todo->MutualRaw == 1) {
			...
			if (ret != OK) {
				if(test_item_check.check_switch == SWITCH_ON){
					test_item_check.node_status[MSRAW_ITEM_CHECK] = ITEM_FAILED;
					test_item_check.node_status[MSRAW_ITEM_CHECK] |= (ret & 0xFFF);
					test_item_check.data[0]= thresholds[0];
					test_item_check.data[1]= thresholds[1];
				}
				logError(1,
				...
				if(test_item_check.check_switch == SWITCH_ON){
					test_item_check.node_status[MSRAW_ITEM_CHECK] = ITEM_OK;
				}
				logError(0,
		...
		if (todo->MutualRawEachNode == 1) {
			ret = parseProductionTestLimits(path_limits,&limit_file,
							MS_RAW_EACH_NODE_MIN,&thresholds_min, &trows,&tcolumns);
			if (ret < OK || (trows !=
					 msRawFrame.header.force_node ||
					 tcolumns != msRawFrame.header.sense_node)) {
				logError(1,
			...
			ret = parseProductionTestLimits(path_limits,&limit_file,
							MS_RAW_EACH_NODE_MAX,&thresholds_max, &trows,&tcolumns);
			if (ret < OK || (trows !=
					 msRawFrame.header.force_node ||
					 tcolumns != msRawFrame.header.sense_node)) {
				logError(1,
			...
			ret = checkLimitsMapTotal(msRawFrame.node_data,msRawFrame.header.force_node,
				     msRawFrame.header.sense_node,thresholds_min, thresholds_max);
			if (ret != OK) {
				logError(1,
	...
	if (todo->MutualKeyRaw == 1) {
		logError(0, "%s .....MS KEY RAW TEST ...enter:\n", tag);
...
int production_test_ms_cx(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	if (( todo->MutualCx2LP | todo->MutualCx2AdjLP ) == 1) {
...
int production_test_ms_cx_lp(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	if (todo->MutualCx2LP == 1) {
		...
		if (ret != OK) {
			if(test_item_check.check_switch == SWITCH_ON){
				test_item_check.node_status[MSCX2LP_ITEM_CHECK] = ITEM_FAILED;
				test_item_check.node_status[MSCX2LP_ITEM_CHECK] |= (ret &0xFFF);
			}
			logError(1,
			...
			if(test_item_check.check_switch == SWITCH_ON){
				test_item_check.node_status[MSCX2LP_ITEM_CHECK] = ITEM_OK;
			}
			logError(0,
	...
	if (todo->MutualCx2AdjLP == 1) {
		...
		if (ret != OK) {
			if(test_item_check.check_switch == SWITCH_ON){
				test_item_check.node_status[MSCX2ADJLP_ITEM_CHECK] = ITEM_FAILED;
				test_item_check.node_status[MSCX2ADJLP_ITEM_CHECK] |= ((ret& 0x3FF) | 0x400);
			}
			logError(1,
			...
			test_item_check.node_status[MSCX2ADJLP_ITEM_CHECK] = ITEM_OK;
			logError(0,
		...
		if (ret != OK) {
			if(test_item_check.check_switch == SWITCH_ON){
				test_item_check.node_status[MSCX2ADJLP_ITEM_CHECK] = ITEM_FAILED;
				test_item_check.node_status[MSCX2ADJLP_ITEM_CHECK] |= ((ret& 0x3FF) | 0x800);
			}
			logError(1,
...
int production_test_ss_raw(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	if (todo->SelfForceRaw == 1 || todo->SelfForceRawGap == 1) {
		...
		if (todo->SelfForceRaw == 1) {
			...
			if (ret != OK) {
				if(test_item_check.check_switch == SWITCH_ON){
					test_item_check.node_status[SSFORCERAW_ITEM_CHECK] = ITEM_FAILED;
					test_item_check.node_status[SSFORCERAW_ITEM_CHECK] |= (ret &0xFFF);
					test_item_check.data[2]=thresholds[0];
					test_item_check.data[3]=thresholds[1];
				}
				logError(1,
				...
				if(test_item_check.check_switch == SWITCH_ON){
					test_item_check.node_status[SSFORCERAW_ITEM_CHECK] = ITEM_OK;
				}
				logError(0,
	...
	if (todo->SelfSenseRaw == 1 || todo->SelfSenseRawGap == 1) {
		...
		if (todo->SelfSenseRaw == 1) {
			...
			if (ret != OK) {
				if(test_item_check.check_switch == SWITCH_ON){
					test_item_check.node_status[SSSENSERAW_ITEM_CHECK] = ITEM_FAILED;
					test_item_check.node_status[SSSENSERAW_ITEM_CHECK] |= (ret &0xFFF);
					test_item_check.data[4]=thresholds[0];
					test_item_check.data[5]=thresholds[1];
				}
				logError(1,
				...
				if(test_item_check.check_switch == SWITCH_ON){
					test_item_check.node_status[SSSENSERAW_ITEM_CHECK] = ITEM_OK;
				}
				logError(0,
			     ...
			     TestToDo *todo)
{
	...
	if (todo->SelfForceIxTotal == 1 || todo->SelfForceIxTotalAdj == 1) {
		...
		if (todo->SelfForceIxTotal == 1) {
			...
			if (ret != OK) {
				if(test_item_check.check_switch == SWITCH_ON){
					test_item_check.node_status[SSFORCEIXTOTAL_ITEM_CHECK] = ITEM_FAILED;
					test_item_check.node_status[SSFORCEIXTOTAL_ITEM_CHECK] |= (ret &0xFFF);
				}
				logError(1,
				...
				if(test_item_check.check_switch == SWITCH_ON){
					test_item_check.node_status[SSFORCEIXTOTAL_ITEM_CHECK] = ITEM_OK;
				}
				logError(0,
	...
	if (todo->SelfSenseIxTotal == 1 || todo->SelfSenseIxTotalAdj == 1) {
		...
		if (todo->SelfSenseIxTotal == 1) {
			...
			if (ret != OK) {
				if(test_item_check.check_switch == SWITCH_ON){
					test_item_check.node_status[SSSENSEIXTOTAL_ITEM_CHECK] = ITEM_FAILED;
					test_item_check.node_status[SSSENSEIXTOTAL_ITEM_CHECK] |= (ret &0xFFF);
				}
				logError(1,
				...
				if(test_item_check.check_switch == SWITCH_ON){
					test_item_check.node_status[SSSENSEIXTOTAL_ITEM_CHECK] = ITEM_OK;
				}
				logError(0,
