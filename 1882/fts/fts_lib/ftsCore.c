...
int fts_system_reset(void)
{
	...
	for (i = 0; i < RETRY_SYSTEM_RESET && res < 0; i++) {
		resetErrorList();
		fts_disableInterruptNoSync();	/* disable interrupt before resetting to
...
int setFeatures(u8 feat, u8 *settings, int size)
{
	...
	logError(1, "fts: setFeatures [0x%x,0x%x,0x%x,0x%x]\n", cmd[2], cmd[3], cmd[4], cmd[5]);
...
int readSysInfo(int request)
{
	...
	index += 3 + 8; /* +3 remaining from mp flag address */
